  ...
  <div>
    ...
    <el-card>
      <div v-show="isShowList">
        <!-- disabled UI库里的禁用属性，这快用的目的是，上面的三级选择如果没有的话，这里就为true,表示添加框禁用 -->
        <el-button
      ...
      <!-- 与上面几行代码实现功能相同  这里还原了一下sync语法糖的实现原理 
      <SpuForm
        ref="spuForm"
        @success="successSpuForm"
        :category3Id="category3Id"
...
export default {
  ...
  methods: {
    ...
    async getSpuList(page) {
      // console.log('发送请求中');
      const {data: { total, records },} = await this.$API.spu.getSpuList(
        page ? page : this.page,
        this.limit,
        this.category3Id
      );
