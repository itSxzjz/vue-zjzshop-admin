  ...
  <div>
    <el-form :model="spuForm" label-width="80px">
      ...
      <el-form-item label="SPU描述">
        <!--  type="textarea" js原生的文本域，可以输入很多文字 rows="4" 高度为4行  -->
        <el-input
      ...
      <el-form-item label="SPU图片">
        <!-- el-upload 图片墙UI -->
        <!-- list-type="picture-card" 上传遍历出多张图片，这个属性不能丢 -->
        <el-upload
      ...
      <el-form-item label="销售属性">
        <el-select
          ...
          :placeholder="
            unUseSaleAttrList.length? `还有${unUseSaleAttrList.length}个未选中`: '无属性了'"
        >
          <el-option
...
export default {
  ...
  methods: {
    ...
    handleRemove(file, fileList) {
      ...
      // 注意:别去修改所有图片的数组,只需要将最新的剩余数组保存入spuForm的spuImageList中即可
    ...
    initUpdateSpuForm(row) {
      ...
      // console.log(row); 
      // 发送四个请求  异步出去请求  4个同时出去请求
      this.getSpuInfo(row.id); // 请求列表信息
      this.getSpuImageList(row.id); // 请求列表图片信息
      this.getBaseSaleAttrList(); // 请求属性信息
      this.getTradeMarks();   // 请求TradeMark列表
    },

    async getSpuImageList(id) {
      const { data } = await this.$API.sku.getSpuImageList(id);
      // console.log(result)  可以请求到详细数据
      // 现在的图片对象结构
      // {
      //   id: 12;
    ...
    handleInputConfirm(row) {
      ...
      //属性值如果重复,也提示用户  some 用some遍历，只要有一个就为true
    ...
    async save() {
      ...
      //2.整理数据
      //2.1 三级分类id spuForm中的三级id必须等于父级传下来的id
