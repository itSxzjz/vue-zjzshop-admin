<template>
  <div>
    ...
    <!-- 
       ...
       如果想要自定义列显示内容,需要使用到作用域插槽,解构出每一行（row）的数据，给column组件内部传入template结构，
      -->
    ...
    <!-- 
      ...
      total->用来指定当前总条数
      @size-change -> pageSize改变的时候触发（具体显示几条数据的设置选项事件）
      @current-change -> currentPage 改变时会触发 （用户点击第几页事件）
      -->
    <el-pagination
    ...
    >
      <el-form :model="tmForm" :rules="rules" ref="addForm">
        ...
        <el-form-item label="品牌LOGO" prop="logoUrl" label-width="100px">
          <!--从接口文档中找到地址，添加到action处。 
          此处的action需要填写图片上传的网络路径,但是记得加上前缀/dev-api ，
          直接上传肯定跨域，所以要加上前缀，经过我们的开发服务器（代理服务器），
          从而成功上传到真正的java服务器-->
          <el-upload
...
export default {
  ...
  // 页面挂载
  mounted() {
    // this.getTradeMarkList  =>  Vue.prototype.getTradeMarkList  结果找不到
  ...
  methods: {
    ...
    async getTradeMarkList(page) {
      const {data: { total, records }} = await this.$API.trademark.getTradeMarkList( page ? page : this.page, this.limit);
    ...
    handleAvatarSuccess(res, file) {
      ...
      // 将res收集到的网络地址，放在tmFrom中的logoUrl中。
    ...
    showDialog(row) {
      ...
      // console.log("row", row); // row上面有id属性，所以可以通过id可以区分是 点击的添加属性还是修改属性
      this.dialogFormVisible = true;
    },

    // 用户点击 确定 的方法回调
    save() {
      //主动校验 ->首先:先校验数据的合法性,数据没问题在发送请求
      ...
      this.$refs.addForm.validate(async (valid) => {
        ...
        if (valid) {
          ...
          try {
            ...
            //3.4清空当前dialog的显示数据，因为收集到的数据都保存在tmFrom上
            this.tmForm = {
              tmName: "",
              logoUrl: "",
            };
    ...
    // 用户点击 取消 的方法回调。dialogFormVisible用来控制弹窗的显示/隐藏
    cancel() {
      this.dialogFormVisible = false;
      this.tmForm = {
        tmName: "",
