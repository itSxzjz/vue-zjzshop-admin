<template>
  <div>
    <el-card style="margin: 20px 0">
      <!-- CategorySelector类别选择器，添加这个组件是因为，有好几个二级路由页面都需要这个模块，所以单独抽离出去，写成一个组件 -->
      <CategorySelector
      ...
      <div v-show="isShowList">
        ...
        <el-table :data="attrList" style="width: 100%; margin-top: 20px" border>
          ...
          <el-table-column label="属性值名称">
              <!-- 插槽 解构赋值拿到row值,slot-scope已经淘汰，取而代之的是v-slot -->
            <template v-slot="{ row }"> 
              <el-tag
      ...
      <div v-show="!isShowList">
        ...
        >
          ...
          <el-table-column prop="prop" label="属性值名称">
            <template v-slot="{ row }">
               <!-- v-show="row.isEdit" 有问题哦，因为它只简析一次模板 -->
              <el-input
                v-if="row.isEdit"
                ref="editInput"
...
export default {
  ...
  methods: {
    ...
    toLook(row) {
      ...
      // 如果用户输入的数据已经存在(返回值应该是布尔值类型，用some方法遍历),不允许进入展示模式
    ...
    toEdit(row) {
      ...
      // $nextTick的回调函数会在.then中执行  和上面的哪行代码不再是同步执行，而是异步执行
      this.$nextTick(() => {
    ...
    async save() {
      ...
      try {
        //3.发送请求
        await this.$API.attr.addOrUpdata(attrForm);
        //4.成功做什么
        //4.1 返回列表页
        this.isShowList = true;
